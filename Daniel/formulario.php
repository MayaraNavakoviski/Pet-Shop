...
if (isset($_POST["nome"])) {
    ...
    // Verificar se já existe um serviço para este animal (mesmo nome e mesmo dono)
    $sql = "SELECT * FROM animal WHERE nome = ? AND dono = ?";
    $stm = $con->prepare($sql);
    $stm->execute([$nome, $dono]);
    $existe = $stm->fetchAll();

    if (count($existe) > 0) {
        array_push($erros, 'Um serviço já foi marcado para este animal com este dono!');
    }
    ...
    <table class="table-auto border border-white text-white m-auto">
        ...
        <?php foreach ($animais as $a) : ?> <!-- O : é pra não usar chave -->
            ...
            <tr>
                ...
                <td>
                    <a href=' excluir.php?id=<?= $a["id"] ?>' onclick="return confirm('Confirma a exclusão?')"><button>Excluir</button></a> <!-- dentro das tags usar aspas simples '' -->
                </td>
            </tr>
        ...
        <div class="bg-gray-500 rounded-xl p-2">
            ...
            <div class="p-4">
                <form class="pt-4" method="POST"
                    ...
                    <input class="input rounded-md bg-gray-200 placeholder-gray-500 ml-2 pl-2" type="number" name="numero" placeholder="Número para contato" value="<?php echo $numero ?>" />
                    <br><br>
                    <!-- para fazer o formato do número para o contato precisamos usar o js, com o regex-->
